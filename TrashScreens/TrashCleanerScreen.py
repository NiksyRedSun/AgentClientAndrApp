...
class TrashCleanerScreen(Screen):
    def __init__(self, **kwargs):
        super(TrashCleanerScreen, self).__init__(**kwargs)
        ...
        self.grid.add_widget(Label(text="[color=03A062]Исполнитель уборки[/color]", size_hint=(1, .10), max_lines=10,
    ...
    def screen_transition_to_contract(self, *args):
        ...
        target = get_agent(self.manager.get_screen("TrashContractScreen").target)
        agent_kills = get_agent_contracts(args[0])
        target_kills = get_agent_contracts(target['id'])

        info = f"[color=03A062]Уборщик: {agent['nickname']}\n" \
               f"Имя: {agent['first_name']} {agent['last_name']}\n" \
               f"Опыт: {agent['exp']}\n" \
               f"Уровень снаряжения: {agent['equipment_level']}\n" \
               f"Предшествующих контрактов: {len(agent_kills)}\n\n" \
               f"Мусор: {target['nickname']}\n" \
               f"Имя: {target['first_name']} {target['last_name']}\n" \
               f"Опыт: {target['exp']}, Деньги: {target['money']}\n" \
               f"Уровень снаряжения: {target['equipment_level']}\n" \
               f"Предшествующих контрактов: {len(target_kills)}\n\n[/color]" \

        self.manager.get_screen("TrashContractScreen").info_label.text = info
        self.manager.get_screen("TrashContractScreen").agent = args[0]
        self.manager.current = "TrashContractScreen"
